...
def affine_forward(x, w, b):
    ...
    ###########################################################################
    x_rows=np.reshape(x,(batch_size,-1))
    out=x_rows.dot(w)+b
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        normalised=(x-sample_mean)/np.sqrt(sample_var+eps)
        out=(gamma*normalised)+beta
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache={'x':x,'sample_mean':sample_mean,'sample_var':sample_var,'normalised':normalised,'gamma':gamma,'eps':eps}
...
def batchnorm_backward(dout, cache):
    ...
    normalised=cache.get('normalised')
    eps=cache.get('eps')
    dbeta=np.sum(dout,axis=0)
    dgamma=np.sum(dout*normalised,axis=0)
    dxhat=dout*gamma
